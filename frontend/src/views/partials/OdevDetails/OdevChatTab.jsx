...
function OdevChatTab({ odev, fetchOdevDetail }) {
  const [createMessage, setCreateMessage] = useState({ title: "", message: "" });
  const [filteredQuestions, setFilteredQuestions] = useState(odev.question_answers || []);
  ...
  useEffect(() => {
    setFilteredQuestions(odev.question_answers || []);
  }, [odev]);
  ...
  const handleSendMessage = async (e) => {
    ...
    formData.append("odev_id", odev?.id);
  ...
  const handleSaveQuestion = async (e) => {
    ...
    try {
      await useAxios().post(
        `eskepstajer/question-answer-list-create/${odev?.id}/`,
        formData
      );
