...
function DersSonuRaporuNotesTab({ derssonuraporu, fetchDersSonuRaporuDetail, id }) {
    ...
    const handleCreateSubmit = async (e) => {
        ...
        formData.append("derssonuraporu_id", id);
        formData.append("title", createNote.title);
        formData.append("note", createNote.note);

        try {
            await useAxios().post(`eskepstajer/derssonuraporu-note/${user?.user_id}/${id}/`, formData);
            console.log("✅ Not eklendi, fetchDersSonuRaporuDetail çağrılıyor");
            await fetchDersSonuRaporuDetail(); // await önemli olabilir
            Swal.fire({
                icon: "success",
                title: "Başarılı!",
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            await useAxios().patch(
                `eskepstajer/derssonuraporu-note-detail/${user?.user_id}/${id}/${selectedNote.id}/`,
                formData
            );
    ...
    const handleDelete = async (selectedNoteid) => {
        try {
            await useAxios().delete(
                `eskepstajer/derssonuraporu-note-detail/${user?.user_id}/${id}/${selectedNoteid}/`
            );
            fetchDersSonuRaporuDetail();
            Swal.fire({
                icon: "success",
    ...
    return (
        <>
            ...
            {derssonuraporu?.notes?.length > 0 ? (
                derssonuraporu.notes.map((note) => (
...
export default DersSonuRaporuNotesTab;
