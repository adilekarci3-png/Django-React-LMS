...
function DersSonuRaporuChatTab({ derssonuraporu, fetchDersSonuRaporuDetail }) {
  if (!derssonuraporu) {
    return <div>Yükleniyor...</div>;
  }
  
  const [createMessage, setCreateMessage] = useState({ title: "", message: "" });
  const [filteredQuestions, setFilteredQuestions] = useState(derssonuraporu.question_answers|| []);
  ...
  useEffect(() => {
    debugger;
    console.log(derssonuraporu);
    setFilteredQuestions(derssonuraporu.question_answers || []);
  }, [derssonuraporu]);
  ...
  const handleSearchQuestion = (e) => {
    ...
    if (!query) {
      ...
      const filtered = derssonuraporu.question_answers?.filter((q) =>
  ...
  const handleSendMessage = async (e) => {
    ...
    formData.append("derssonuraporu_id", derssonuraporu?.id);
  ...
  const handleSaveQuestion = async (e) => {
    ...
    try {
      await useAxios().post(
        `eskepstajer/question-answer-list-create/${derssonuraporu?.id}/`,
        formData
      );
...
export default DersSonuRaporuChatTab;
