...
function EskepOgrenciDersSonuRaporus() {
  ...
  const fetchData = async () => {
    ...
    setFetching(true);
    try {
      const res = await api.get(`eskepogrenci/derssonuraporu-list/${user.user_id}/`);
      setItems(res.data);
    } finally {
      setFetching(false);
    }
  ...
  const collectSections = (c) => {
    // Raporlarda muhtemel alan isimleri
    ...
    // Yedek olarak odev yapısına benzerse:
              ...
              {!fetching && (
                <div className="card mb-4">
                  <div className="card-header">
                    ...
                    <span>Panel sayfanızdan raporlarınızı inceleyebilirsiniz.</span>
                  </div>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (window.confirm("Bu raporu silmek istediğinize emin misiniz?")) {
                                      api.delete(`/eskepogrenci/derssonuraporu/${c.id}/`).then(() => fetchData());
                                    }
                                  }}
                                >
                                  Sil
