...
function AgentHafizBilgiList() {
  const [hafizList, setHafizList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [currentItem, setCurrentItem] = useState(null);
  const [showEditModal, setShowEditModal] = useState(false);
  const [showDetailModal, setShowDetailModal] = useState(false);
  const rehydrated = useAuthStore((state) => state.rehydrated);
  const [isLoggedIn, user, roleData] = useAuthStore((state) => [
    state.isLoggedIn,
    state.user,
    state.roleData,
  ]);
  ...
  useEffect(
    () => {
      ...
      const fetchData = async () => {
        try {
          ...
          if (
            roleData?.base_roles?.includes("Agent") &&
            roleData?.sub_roles?.includes("HBSTemsilci")
          ) {
            const agentInfo = await api.get(`agent/${agentId}/`);

            if (agentInfo.data.is_agent) {
              const hafizRes = await api.get(
                `/hafizbilgi/list/${agentInfo.data.agent_id}`
              );
              setHafizList(hafizRes.data);
            }
          }
        } catch (err) {
    ...
    [isLoggedIn, user, roleData,rehydrated]
  );
