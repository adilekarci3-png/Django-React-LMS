// src/pages/EskepStajerKitapTahliliDetail.jsx
// Route: /eskepstajer/kitaptahlileris/:id/:koordinator_id/
//
// API (Swagger'a göre):
//   GET    eskepstajer/kitaptahlili-detail/{user_id}/{id}/
//   POST   eskepstajer/kitaptahlili-note/{koordinator_id}/{id}/
//   PATCH  eskepstajer/kitaptahlili-note/{koordinator_id}/{id}/
//   DELETE eskepstajer/kitaptahlili-note/{koordinator_id}/{id}/
//   GET/POST eskepstajer/question-answer-list-create/{odev_id}/  ← kitap için aynı endpoint
...
function EskepStajerKitapTahliliDetail() {
  const api = useAxios();
  const userData = useUserData();
  const { id, koordinator_id } = useParams();
  ...
  const fetchDetail = async () => {
    ...
    try {
      setFetching(true);
      const res = await api.get(`eskepstajer/kitaptahlili-detail/${userData.user_id}/${id}/`);
      const data = res.data || {};
      setDetail(data);
  ...
  const handleMarkLessonAsCompleted = async (variantItemId) => {
    try {
      ...
      fd.append("kitaptahlili_id", id);
      fd.append("variant_item_id", variantItemId);
      await api.post(`eskepstajer/kitaptahlili-completed/`, fd);
      await fetchDetail();
  ...
  const handleSubmitNote = async (e) => {
    ...
    if (selectedNote?.id) fd.append("note_id", selectedNote.id);
    try {
      if (selectedNote?.id) {
        await api.patch(`eskepstajer/kitaptahlili-note/${koordinator_id}/${id}/`, fd);
        Toast().fire({ icon: "success", title: "Not güncellendi" });
      } else {
        await api.post(`eskepstajer/kitaptahlili-note/${koordinator_id}/${id}/`, fd);
        Toast().fire({ icon: "success", title: "Not eklendi" });
      }
      await fetchDetail();
      handleNoteClose();
    } catch (e) { Toast().fire({ icon: "error", title: "Not kaydedilemedi" }); }
  };
  const handleDeleteNote = async (noteId) => {
    try {
      await api.delete(`eskepstajer/kitaptahlili-note/${koordinator_id}/${id}/`, { data: { note_id: noteId } });
      await fetchDetail();
      Toast().fire({ icon: "success", title: "Not silindi" });
    } catch (e) { Toast().fire({ icon: "error", title: "Not silinemedi" }); }
  };
  ...
  useEffect(() => { if (lastElementRef.current) lastElementRef.current.scrollIntoView({ behavior: "smooth" }); }, [selectedConversation]);
  ...
  const handleCreateReviewSubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData();
    fd.append("kitaptahlili_id", detail?.kitaptahlili?.id || detail?.id);
    fd.append("user_id", userData?.user_id);
    fd.append("rating", createReview.rating);
    fd.append("review", createReview.review);
    await api.post(`eskepstajer/rate-kitaptahlili/`, fd);
    await fetchDetail();
  ...
  const handleUpdateReviewSubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData();
    fd.append("kitaptahlili", detail?.kitaptahlili?.id || detail?.id);
              ...
              <div className="card border-0 shadow-sm rounded-3 mb-3">
                <div className="card-body p-3 p-md-4">
                  <div className="d-flex align-items-center justify-content-between mb-2">
                    <h5 className="mb-0">Kitap Tahlili Detayı</h5>
                    {fetching && <span className="badge bg-secondary">Yükleniyor</span>}
                  </div>
                  <div className="small text-muted mb-3">{detail?.kitaptahlili?.title || detail?.title || "—"}</div>
                  <div className="mb-3">
              ...
              <div className="card shadow rounded-3 border-0">
                <div className="card-header bg-white border-0 px-3 px-md-4 pt-3 pb-0">
                  <ul className="nav nav-pills gap-2 flex-wrap" role="tablist">
                    {[["#kt-1","Bölümler",true],["#kt-2","Notlar"],["#kt-3","Konuşma"],["#kt-4","Not Ver"]].map(([target,label,active]) => (
                ...
                <div className="card-body p-3 p-md-4">
                  <div className="tab-content">
                    ...
                    <div className="tab-pane fade show active" id="kt-1">
                      {fetching ? <SkeletonCurriculum /> : (
                        <div className="accordion" id="accKt">
                          ...
                          {(detail?.curriculum || []).map((c, index) => (
                            <div className="accordion-item border rounded-3 overflow-hidden mb-3" key={index}>
                              <h2 className="accordion-header">
                                <button className="accordion-button collapsed bg-white fw-semibold" type="button" data-bs-toggle="collapse" data-bs-target={`#col-kt-${c?.variant_id}`}>
                              ...
                              <div id={`col-kt-${c?.variant_id}`} className="accordion-collapse collapse" data-bs-parent="#accKt">
                                <div className="accordion-body bg-light">
                                  {(c?.variant_items||[]).map((l,idx)=>{
                                    const itemId=l?.id??l?.variant_item_id;const completed=isItemCompleted(l);
                                    return (
                    ...
                    <div className="tab-pane fade" id="kt-2">
                          ...
                          {(detail?.notes||[]).map((n)=>(
                            <div key={n?.id} className="border rounded-3 p-3 bg-white shadow-sm">
                              <div className="d-flex justify-content-between align-items-start">
                                <div><h6 className="mb-1">{n?.title}</h6><p className="mb-0 text-muted">{n?.note||n?.notes}</p></div>
                    ...
                    <div className="tab-pane fade" id="kt-3">
                    ...
                    <div className="tab-pane fade" id="kt-4">
                      <div className="card border-0"><div className="card-body p-0">
      ...
      <Modal show={noteShow} size="lg" onHide={handleNoteClose} centered>
        ...
        <Modal.Body>
          <form onSubmit={handleSubmitNote}>
            <div className="mb-3"><label className="form-label">Not Başlığı</label><input value={createNote.title} name="title" onChange={handleNoteChange} type="text" className="form-control" required/></div>
            <div className="mb-3"><label className="form-label">İçerik</label><textarea value={createNote.note} name="note" onChange={handleNoteChange} className="form-control" rows={8} required/></div>
      ...
      <Modal show={questionModal} size="lg" onHide={()=>setQuestionModal(false)} centered>
        ...
        <Modal.Body>
          <form onSubmit={handleSaveQuestion}>
            <div className="mb-3"><label className="form-label">Soru Başlığı</label><input value={createMessage.title} name="title" onChange={handleMessageChange} type="text" className="form-control" required/></div>
            <div className="mb-3"><label className="form-label">Mesaj</label><textarea value={createMessage.message} name="message" onChange={handleMessageChange} className="form-control" rows={8} required/></div>
            <div className="d-flex justify-content-end gap-2">
...
export default EskepStajerKitapTahliliDetail;
function EmptyState({title="Kayıt bulunamadı",subtitle=""}){return(<div className="text-center p-4 border rounded-3 bg-white"><div className="display-6 mb-2">🗂️</div><h6 className="mb-1">{title}</h6>{subtitle&&<div className="text-muted small">{subtitle}</div>}</div>);}
function SkeletonCurriculum(){return <div className="vstack gap-2">{Array.from({length:4}).map((_,i)=><div key={i} className="p-3 bg-white border rounded-3"><div className="placeholder-glow"><span className="placeholder col-6"></span></div></div>)}</div>;}
function SkeletonNotes(){return <div className="vstack gap-2">{Array.from({length:3}).map((_,i)=><div key={i} className="p-3 bg-white border rounded-3"><div className="placeholder-glow"><span className="placeholder col-5"></span></div></div>)}</div>;}