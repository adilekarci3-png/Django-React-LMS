// src/pages/EskepStajerDersSonuRaporuDetail.jsx
// Route: /eskepstajer/derssonuraporus/:id/:koordinator_id/
//
// API ÖZET (Swagger'a göre):
//   GET    eskepstajer/derssonuraporu-detail/{user_id}/{id}/
//   POST   eskepstajer/derssonuraporu-note/{koordinator_id}/{id}/
//   PATCH  eskepstajer/derssonuraporu-note/{koordinator_id}/{id}/
//   DELETE eskepstajer/derssonuraporu-note/{koordinator_id}/{id}/
//   GET    eskepstajer/question-answer-list-create/{odev_id}/   ← DSR için ayrı endpoint yoksa odev_id yerine dsr id kullanılır
...
function EskepStajerDersSonuRaporuDetail() {
  const api = useAxios();
  ...
  const fetchDetail = async () => {
    ...
    try {
      setFetching(true);
      const res = await api.get(`eskepstajer/derssonuraporu-detail/${userData.user_id}/${id}/`);
      const data = res.data || {};
      setDetail(data);
  ...
  const handleMarkLessonAsCompleted = async (variantItemId) => {
    try {
      ...
      fd.append("dersSonuRaporu_id", id);
      fd.append("variant_item_id", variantItemId);
      await api.post(`eskepstajer/derssonuraporu-completed/`, fd);
      await fetchDetail();
  ...
  const handleSubmitNote = async (e) => {
    ...
    // Eğer backend note_id bekliyorsa:
    if (selectedNote?.id) fd.append("note_id", selectedNote.id);
    try {
      if (selectedNote?.id) {
        await api.patch(`eskepstajer/derssonuraporu-note/${koordinator_id}/${id}/`, fd);
        Toast().fire({ icon: "success", title: "Not güncellendi" });
      } else {
        await api.post(`eskepstajer/derssonuraporu-note/${koordinator_id}/${id}/`, fd);
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      // note_id body'de veya query'de geçebilir — backend'e göre ayarlayın
      await api.delete(`eskepstajer/derssonuraporu-note/${koordinator_id}/${id}/`, {
        data: { note_id: noteId },
      });
      await fetchDetail();
  ...
  // Swagger'da "odev_id" yazıyor ama DSR sayfası için de aynı endpoint
  // kullanılıyorsa id gönderiyoruz. Farklı bir endpoint varsa güncelleyin.
  ...
  const handleCreateReviewSubmit = async (e) => {
    ...
    await api.post(`eskepstajer/rate-derssonuraporu/`, fd);
    await fetchDetail();
            ...
            <div className="col-lg-8 col-xl-9">
              ...
              <div className="card border-0 shadow-sm rounded-3 mb-3">
                <div className="card-body p-3 p-md-4">
                  ...
                  <div className="small text-muted mb-3">{detail?.derssonuraporu?.title || detail?.title || "—"}</div>
                  <div className="mb-3">
              ...
              <div className="card shadow rounded-3 border-0">
                <div className="card-header bg-white border-0 px-3 px-md-4 pt-3 pb-0">
                  <ul className="nav nav-pills gap-2 flex-wrap" role="tablist">
                    {[["#dsr-1","Bölümler",true],["#dsr-2","Notlar"],["#dsr-3","Konuşma"],["#dsr-4","Not Ver"]].map(([target,label,active]) => (
                          ...
                          {(detail?.curriculum || []).map((c, index) => (
                            <div className="accordion-item border rounded-3 overflow-hidden mb-3" key={index}>
                              <h2 className="accordion-header">
                                <button className="accordion-button collapsed bg-white fw-semibold" type="button" data-bs-toggle="collapse" data-bs-target={`#col-${c?.variant_id}`}>
...
export default EskepStajerDersSonuRaporuDetail;
