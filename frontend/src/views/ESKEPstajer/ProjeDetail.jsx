// src/pages/EskepStajerProjeDetail.jsx
// Route: /eskepstajer/projes/:id/:koordinator_id/
//
// API (Swagger'a göre):
//   GET    eskepstajer/proje-detail/{user_id}/{id}/
//   POST   eskepstajer/proje-note/{koordinator_id}/{id}/
//   PATCH  eskepstajer/proje-note/{koordinator_id}/{id}/
//   DELETE eskepstajer/proje-note/{koordinator_id}/{id}/
...
function EskepStajerProjeDetail() {
  const api = useAxios();
  ...
  const fetchDetail = async () => {
    ...
    try {
      setFetching(true);
      const res = await api.get(`eskepstajer/proje-detail/${userData.user_id}/${id}/`);
      const data = res.data || {};
      setDetail(data);
  ...
  const handleMarkLessonAsCompleted = async (variantItemId) => {
    try {
      ...
      fd.append("proje_id", id);
      fd.append("variant_item_id", variantItemId);
      await api.post(`eskepstajer/proje-completed/`, fd);
      await fetchDetail();
  ...
  const handleSubmitNote = async (e) => {
    ...
    try {
      if (selectedNote?.id) {
        await api.patch(`eskepstajer/proje-note/${koordinator_id}/${id}/`, fd);
        Toast().fire({ icon: "success", title: "Not güncellendi" });
      } else {
        await api.post(`eskepstajer/proje-note/${koordinator_id}/${id}/`, fd);
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      await api.delete(`eskepstajer/proje-note/${koordinator_id}/${id}/`, { data: { note_id: noteId } });
      await fetchDetail();
  ...
  const handleCreateReviewSubmit = async (e) => {
    ...
    await api.post(`eskepstajer/rate-proje/`, fd);
    await fetchDetail();
  ...
  const handleUpdateReviewSubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData();
    fd.append("proje", detail?.proje?.id || detail?.id);
              ...
              <div className="card border-0 shadow-sm rounded-3 mb-3">
                <div className="card-body p-3 p-md-4">
                  <div className="d-flex align-items-center justify-content-between mb-2">
                    <h5 className="mb-0">Proje Detayı</h5>
                    {fetching && <span className="badge bg-secondary">Yükleniyor</span>}
                  </div>
                  <div className="small text-muted mb-3">{detail?.proje?.title || detail?.title || "—"}</div>
                  <div className="mb-3">
              ...
              <div className="card shadow rounded-3 border-0">
                <div className="card-header bg-white border-0 px-3 px-md-4 pt-3 pb-0">
                  <ul className="nav nav-pills gap-2 flex-wrap" role="tablist">
                    {[["#pr-1","Bölümler",true],["#pr-2","Notlar"],["#pr-3","Konuşma"],["#pr-4","Not Ver"]].map(([target,label,active]) => (
                ...
                <div className="card-body p-3 p-md-4">
                  <div className="tab-content">
                    ...
                    <div className="tab-pane fade show active" id="pr-1">
                      {fetching ? <SkeletonCurriculum /> : (
                        <div className="accordion" id="accPr">
                          ...
                          {(detail?.curriculum || []).map((c, index) => (
                            <div className="accordion-item border rounded-3 overflow-hidden mb-3" key={index}>
                              <h2 className="accordion-header">
                                <button className="accordion-button collapsed bg-white fw-semibold" type="button" data-bs-toggle="collapse" data-bs-target={`#col-pr-${c?.variant_id}`}>
                              ...
                              <div id={`col-pr-${c?.variant_id}`} className="accordion-collapse collapse" data-bs-parent="#accPr">
                    ...
                    <div className="tab-pane fade" id="pr-2">
                    ...
                    <div className="tab-pane fade" id="pr-3">
                    ...
                    <div className="tab-pane fade" id="pr-4">
...
export default EskepStajerProjeDetail;
