...
function ProjeEdit() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const v = (d.curriculum || []).map((c) => {
          ...
          return {
            id: c.id,                               // DOĞRU: curriculum (VariantProje) id
  ...
  const validate = () => {
    const er = {};
    if (!proje.title) er.title = "Ödev başlığı zorunludur.";
    if (!proje.description) er.description = "Ödev açıklaması zorunludur.";
