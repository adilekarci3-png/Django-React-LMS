...
#             .select_related(
#                 "inserteduser",
#                 "koordinator", "koordinator__user",
#                 "category",
#             )
#             # Eğer gerçekten bir ilişkili isim 'attachments' varsa ekleyin; yoksa kaldırın
#             # .prefetch_related("attachments")
#             .order_by("-date", "-id")
#             .distinct()
#         )

#         # Sadece aktif istenirse:
...
#     def get_queryset(self):
#         # URL'de user_id varsa onu kullan; yoksa giriş yapan user
#         user_id = self.kwargs.get("user_id") or getattr(self.request.user, "id", None)
#         if not user_id:
#             return api_models.KitapTahlili.objects.none()

#         # Kullanıcıya bağlı Koordinator'u bul
#         try:
#             koordinator = api_models.Koordinator.objects.select_related("user").get(user__id=user_id)
#         except api_models.Koordinator.DoesNotExist:
#             return api_models.KitapTahlili.objects.none()

#         # (Opsiyonel) sadece ESKEPStajerKoordinator alt rolü olanlar görebilsin
...
#                 "koordinator", "koordinator__user",
#                 "category",
#                 # OneToOne ise faydalı; FK ise kaldırılabilir
#                 "inserteduser__stajer",
#                 "inserteduser__ogrenci",
#             )
#             .order_by("-date", "-id")
#             .distinct()
#         )

#         # Yalnızca aktif kayıtlar istenirse:
#         # if self.request.query_params.get("active") in ("1", "true", "True"):
#         #     qs = qs.filter(active=True)
...
#         # Item'ları Variant üzerinden önden çek
...
#                 "koordinator", "koordinator__user",
#                 "category",
#                 # OneToOne ise işe yarar; değilse kaldırın
#                 "inserteduser__stajer",
#                 "inserteduser__ogrenci",
#             )
#             .prefetch_related(
#                 "variantodev_set",          # Odev -> VariantOdev
#                 prefetch_items,             # VariantOdev -> VariantOdevItem (variantOdev_items)
#             )
#             .order_by("-date", "-id")
#             .distinct()
#         )
...
#     """
#     URL: /eskepinstructor/kitaptahlili-detail/<int:koordinator_id>/<int:kitaptahlili_id>/
#     koordinator_id: Koordinator.id veya Koordinator.user_id olabilir.
#     """
#     serializer_class   = api_serializer.KitapTahliliSerializer
#     permission_classes = [AllowAny]
...
#     # --- object ---
#     def get_object(self):
#         kt_id = self.kwargs.get(self.lookup_url_kwarg)
#         try:
#             kt_id = int(kt_id)
#         except (TypeError, ValueError):
...
#     """
#     URL: .../eskepinstructor/derssonuraporu-detail/<int:koordinator_id>/<int:derssonuraporu_id>/
#     """
#     serializer_class   = api_serializer.DersSonuRaporuSerializer
#     permission_classes = [AllowAny]
#     lookup_url_kwarg   = "derssonuraporu_id"  # URL’deki rapor parametresi

#     def get_queryset(self):
#         # İlk parametre: koordinator_id
#         try:
#             koordinator_id = int(self.kwargs.get("koordinator_id"))
#         except (TypeError, ValueError):
#             raise NotFound("Geçersiz koordinator_id")

#         # İlişkileri tek sorguda almak için select_related
#         return (
#             api_models.DersSonuRaporu.objects
#             .filter(koordinator_id=koordinator_id)
#             .select_related(
#                 "inserteduser",
#                 "koordinator", "koordinator__user",
#                 "category",
...
#     def get_object(self):
#         # İkinci parametre: derssonuraporu_id
#         rapor_id = self.kwargs.get(self.lookup_url_kwarg)
#         try:
#             rapor_id = int(rapor_id)
#         except (TypeError, ValueError):
...
#     """
#     URL: /eskepinstructor/proje-detail/<int:koordinator_id>/<int:proje_id>/
#     koordinator_id: Koordinator.pk veya Koordinator.user_id olabilir
#     """
#     serializer_class   = api_serializer.EskepProjeSerializer  # <- modeline göre
#     permission_classes = [AllowAny]
...
#         koordinator = self._resolve_koordinator()
#         return (
#             api_models.EskepProje.objects.filter(koordinator=koordinator).select_related("inserteduser", "koordinator", "koordinator__user", "category")
#         )

#     def get_object(self):
#         proj_id = self.kwargs.get(self.lookup_url_kwarg)
#         try:
#             proj_id = int(proj_id)
#         except (TypeError, ValueError):
...
#     # ✅ DRF sözleşmesi: queryset + serializer_class zorunlu
#     queryset = api_models.NoteOdev.objects.all()           # <-- kendi modelinizi yazın
#     serializer_class = api_serializer.NoteOdevSerializer   # <-- kendi serializer’ınızı yazın
#     permission_classes = [IsAuthenticated]
#     lookup_field = "pk"  # veya "id" / özel bir url kwarg kullanıyorsanız: lookup_url_kwarg = "note_id"
...
#         # Burada gerçek filtrelemenizi yapın (örn. sadece kendi notları):
#         user = self.request.user
#         return api_models.NoteOdev.objects.filter(instructor=user)
...
#     # drf-yasg bazı durumlarda queryset'e de dokunur; tanımlamak güvenli olur:
#     queryset           = api_models.Question_Answer_MessageKitapTahlili.objects.all()

#     def get_queryset(self):
#         # Swagger şema üretimi sırasında gerçek DB erişimini kes
#         if getattr(self, "swagger_fake_view", False):
...
#             api_models.Question_AnswerKitapTahlili,
#             id=qid,
#             kitaptahlili=kt
#         )

#         # 4) Gönderen (sender)
#         sender = request.user if (hasattr(request, "user") and request.user.is_authenticated) else None
#         if not sender:
#             uid = data.get("user_id") or data.get("gonderen_id")
#             if not uid:
#                 return Response({"detail": "user_id (veya gonderen_id) gerekli"}, status=status.HTTP_400_BAD_REQUEST)
...
#         # 6) Oluştur
#         msg = api_models.Question_Answer_MessageKitapTahlili.objects.create(
#             kitaptahlili=kt,
#             question=question,
#             mesajiGonderen=sender,
#             mesajiAlan=receiver,
...
#         return Response(
#             {"ok": True, "question_id": question.id, "message_id": msg.id},
#             status=status.HTTP_201_CREATED
#         )
...
#     # drf-yasg bazı durumlarda queryset erişir; güvenli olması için tanımlıyoruz.
...
#         # derssonuraporu_id URL'den veya body'den gelebilir
...
#         # Gönderen: IsAuthenticated varken request.user kesinlikle mevcut
#         sender = request.user
#         if not sender or not sender.is_authenticated:
#             # Mevcut davranışı korumak isterseniz bu fallback'i bırakın
#             uid = data.get("user_id")
#             if not uid:
#                 return Response({"detail": "user_id gerekli"}, status=status.HTTP_400_BAD_REQUEST)
#             sender = get_object_or_404(User, id=uid)

#         # Alıcı: soru üzerindeki mesajiAlan yoksa DSR'ın inserteduser'ı
#         receiver = getattr(question, "mesajiAlan", None) or getattr(derssonuraporu, "inserteduser", None)
...
#         # Basit cevap — isterseniz bir output serializer kullanabilirsiniz
#         return Response(
#             {"ok": True, "question_id": question.id, "message_id": msg.id},
#             status=status.HTTP_201_CREATED
#         )
...
#     print(data["sub_roles"])
#     print(data["base_roles"])
#     return Response(data)
