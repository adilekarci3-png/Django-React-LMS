...
#         out = []
#         # Odev -> VariantOdev (ters ad: variantodev_set)
#         for v in obj.variantodev_set.all():
#             # VariantOdev -> VariantOdevItem (ters ad: variantOdev_items)
...
#         """DersSonuRaporu → Variant ilişkisini güvenle bul."""
#         # 1) Model metodu varsa onu kullan
#         if hasattr(obj, "curriculum") and callable(getattr(obj, "curriculum")):
#             try:
#                 return obj.curriculum()
#             except Exception:
#                 pass
#         # 2) Yaygın ters adlar (model isimlerinize göre gerekirse genişletin)
#         for attr in [
#             "variants",                      # related_name standard
#             "variant_set",                   # default reverse
#             "variantderssonuraporu_set",     # olası snake
#             "variantDersSonuRaporu_set",     # olası Camel
#         ]:
#             if hasattr(obj, attr):
...
#         VariantSer = api_serializer.VariantSerializer
#         qs = self._get_variants_qs(obj)
#         return VariantSer(qs, many=True, context=self.context).data if qs is not None else []

#     def get_lectures(self, obj):
#         """
#         Tüm variant item'larını flat liste olarak döndür.
#         Variant → Items ilişkisi için yaygın ters adları dener.
#         Son çare: modelde lectures() doğrudan item queryset'i döndürüyorsa onu kullanır.
#         """
#         VariantItemSer = api_serializer.VariantItemSerializer
...
#                 return VariantItemSer(items_qs, many=True, context=self.context).data
#             except Exception:
#                 return []
...
#         for v in variants:
#             for items_attr in [
#                 "items",                            # related_name standard
#                 "variant_items",                    # yaygın alternatif
#                 "variantderssonuraporu_items",      # olası snake
#                 "variantDersSonuRaporu_items",      # olası Camel
#                 "variantderssonuraporuitem_set",    # default reverse olasısı
#             ]:
...
#         return VariantItemSer(collected, many=True, context=self.context).data

#     def get_question_answers(self, obj):
#         qas = api_models.Question_AnswerDersSonuRaporu.objects.filter(derssonuraporu=obj)
#         return api_serializer.Question_AnswerDersSonuRaporuSerializer(
#             qas, many=True, context=self.context
#         ).data      
...
#     # ---------- helpers ----------
#     def _get_variants_qs(self, obj):
#         """KitapTahlili → Variant ilişkisini güvenle bul."""
#         # 1) Model metodu varsa
#         if hasattr(obj, "curriculum") and callable(getattr(obj, "curriculum")):
#             try:
#                 return obj.curriculum()
#             except Exception:
...
#         # 2) Yaygın ters adlar (model isimlerinize göre genişletebilirsiniz)
#         for attr in [
#             "variants",                    # related_name standard
#             "variant_set",                 # default ters ad
#             "variantkitaptahlili_set",     # olası
#             "variantKitapTahlili_set",     # olası (CamelCase model)
#             "variankitaptahlili_set",      # yazım varyasyonları için örnek
#         ]:
#             if hasattr(obj, attr):
#                 return getattr(obj, attr).all()
#         return None
...
#     def get_lectures(self, obj):
#         """
#         Tüm varyant item'larını tek (flat) listede döndürür.
#         Variant → Items ilişkisi için yaygın ters adları dener.
#         Eğer modelde lectures() doğrudan item queryset’i döndürüyorsa onu da kullanır.
#         """
#         VariantItemSer = api_serializer.VariantItemSerializer

#         variants = self._get_variants_qs(obj)
#         # Son çare: model metodu doğrudan item döndürüyorsa
#         if variants is None and hasattr(obj, "lectures") and callable(getattr(obj, "lectures")):
#             try:
#                 items_qs = obj.lectures()
#                 return VariantItemSer(items_qs, many=True, context=self.context).data
...
#                 "items",                          # related_name standard
#                 "variant_items",                  # yaygın alternatif
#                 "variantKitapTahlili_items",      # olası (CamelCase)
#                 "variantkitaptahlili_items",      # olası (snake)
#                 "variantkitaptahliliitem_set",    # default ters ad olasısı
#                 "variantkitaptahlili_items",      # olası
#             ]:
#                 if hasattr(v, items_attr):
#                     try:
#                         collected.extend(list(getattr(v, items_attr).all()))
...
#         # 2) Ters ilişki adlarını sırayla dene
#         for variants_attr in [
#             "variants",                # related_name standard
#             "variant_set",             # default reverse name
#             "variantproje_set",        # olası isim
#             "variantEskepProje_set",   # olası isim (CamelCase)
#             "varianteskepproje_set",   # olası isim (snake)
#         ]:
...
#     # ---- lectures (tüm variant item'ları flat liste) ----
#     def get_lectures(self, obj):
#         VariantItemSer = api_serializer.VariantItemSerializer

#         # Önce variant koleksiyonunu bul
#         variants = None

#         # 1) curriculum() metodu varsa
#         if hasattr(obj, "curriculum") and callable(getattr(obj, "curriculum")):
#             try:
...
#         # 3) Son çare: lectures() doğrudan item döndürüyorsa
#         if variants is None and hasattr(obj, "lectures") and callable(getattr(obj, "lectures")):
#             try:
#                 items_qs = obj.lectures()
#                 return VariantItemSer(items_qs, many=True, context=self.context).data
...
#         # Variant içindeki item koleksiyonlarını topla
#         collected = []
#         for v in variants:
#             for items_attr in [
#                 "items",                     # related_name standard
#                 "variant_items",             # yaygın alternatif
#                 "variantProje_items",        # senaryona göre olası
#                 "variantEskepProje_items",   # senaryona göre olası
#                 "variantprojeitem_set",      # default reverse name olasısı
#                 "varianteskepprojeitem_set", # default reverse name olasısı
#             ]:
#                 if hasattr(v, items_attr):
#                     try:
#                         collected.extend(list(getattr(v, items_attr).all()))
...
#     # Koordinatör isterse başka eğitmen ID'si verebilir (write-only)
...
#     # Yazarken alternatif anahtar adları → videoUrl'e map
#     video_url = serializers.CharField(write_only=True, required=False, allow_blank=True, source="videoUrl")
#     url = serializers.CharField(write_only=True, required=False, allow_blank=True, source="videoUrl")  # ← ek

#     # Okurken frontend için "url" alias'ı (read-only)
...
#             "title",
#             "description",
#             "videoUrl",       # model alanı (camelCase, read)
#             "video_url",      # write-only alias
#             "url",            # write-only alias
#             "url_read",       # read-only alias → frontend "url"
#             "instructor_id",  # write-only
#             "instructor",     # read-only (pk)
#             "owner_name",     # read-only (görünüm için güzel)
#             "created_at",
#         ]
...
#         """
#         Create sırasında videoUrl zorunlu, PATCH'te opsiyonel.
#         (attrs, source mapping sonrası "videoUrl" anahtarını içerir.)
#         """
...
#         """
#         Çıktıyı 'url' anahtarına da yansıt (url_read alias'ını 'url' olarak dışa aktar).
#         Böylece frontend columns: [{ key: 'url', ... }] sorunsuz çalışır.
#         """
#         data = super().to_representation(instance)
#         data["url"] = data.pop("url_read", None)  # url_read → url
#         return data
